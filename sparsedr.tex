...
\section{System Architecture} % Что.
    ...
    % Данная работа основана на статье 2024 года [ссылка], и в основе представляет собой
    % итерационный метод, в котором на каждой итерации происходит отрисовка сцены с
    ...
    % Для корректного расчёта производных по значениям SDF оценивается relaxed boundary
    % integral. Также, спустя некоторое количество шагов, the SDF is upsampled.

    % Теперь о внесённых модификациях. Основным отличием является замена SDF grid на
    % sparse brick set [link]. Данное представление представляет собой a sparse set of bricks --
    % small non-intersecting SDF grids. В нашем случае, размер каждого брика фиксирован
