...
\section{Introduction}
    ...
    \begin{itemize}
    	...
    	\item We provide two novel algorithms of redistancing for sparse SDF: a global version,
              which is used once per epoch, and local version used on every iteration.
    \end{itemize}
...
\end{figure}
    ...
    On the end of each epoch two steps are executed: upsampling step and global redistancing.
    Upsampling is not performed before the resulting model is saved.
...
\textbf{Gradient evaluation.}
    ...
    all derivatives were obtained manually.
    Gradients are accumulated in a float array whose size equals the number of optimized
    parameters. In the shader, when computing the color of a pixel for 16 samples,
    ...
    accurate gradients for smaller relaxed epsilon values.

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth,keepaspectratio]{figures/redistancing.eps}
    \caption{Comparison of the redistancing algorithm on dense grid (on the left) with the local redistancing
             (on the right) on SBS, bricks are 3x3 distances in size.
...
\textbf{Redistancing.}
    ...
    while giving the same result for differentiable rendering, so it is applied on
    each iteration.
